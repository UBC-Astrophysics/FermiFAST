...
void j_dfpmin(double p[],int n,double ftol,int *iter,double *fret,
	      ...
	      double (*fdfunc)(double [], double *, void *), void *localdata)
{
	int j,i,its;
	...
	for (its=1;its<=ITMAX;its++) {
		*iter=its;
		j_linmin(p,xi,n,fret,func,localdata);
	...
	printf("Too many iterations in J_DFPMIN");
}
