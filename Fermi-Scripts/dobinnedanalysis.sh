...
if [ $# -lt 1 ]
    then
    echo dobinnedanalysis.sh _Name_ [  RA DEC Radius [ TMin TMax ] ]
    exit
fi
...
runcmd "gtbin evfile=${1}_gti.fits scfile=Spacecraft.fits outfile=${1}_ccube.fits algorithm=CCUBE ebinalg=LOG emin=100 emax=300000 enumbins=30 nxpix=300 nypix=300 binsz=0.2 coordsys=CEL proj=AIT xref=$xref yref=$yref axisrot=0" ${1}_ccube.fits

runcmd "gtltcube evfile=${1}_gti.fits scfile=Spacecraft.fits outfile=${1}_ltcube.fits dcostheta=0.025 binsz=1" ${1}_ltcube.fits

runcmd "gtexpcube2 infile=${1}_ltcube.fits cmap=none outfile=${1}_expcube.fits irfs=P7REP_SOURCE_V15 xref=$xref yref=$yref nxpix=360 nypix=360 binsz=0.25 coordsys=CEL axisrot=0 proj=AIT emin=100 emax=300000 enumbins=30" ${1}_expcube.fits

runcmd "gtsrcmaps cmap=${1}_ccube.fits expcube=${1}_ltcube.fits srcmdl=${1}_input_model.xml bexpmap=${1}_expcube.fits outfile=${1}_srcmaps.fits irfs=P7REP_SOURCE_V15 emapbnds=no" ${1}_srcmaps.fits
