...
function (\texttt{P7REP\_SOURCE\_V15}) and the PASS 7 reprocessed data.
From the form of the likelihood function, Eq.~\ref{eq:2}, we do not
use a model for the background and the key ingredient of the instrument response
if the estimate of the point-spread function.
...
\begin{figure*}
  ...
  \caption{Sources with $\ln P(\log L)<-12.5$ (i.e. five sigma) in the
    1~GeV catalogue (small blue circles), in the 100~GeV catalogue
...
FermiFAST catalogue and why.  Fig.~\ref{fig:fermi_comp} in the left
panel shows the relationship between the significances of a source
...
catalogue and all of the 3PSC sources in the right panel of
Fig.~\ref{fig:fermi_comp}.  We see that FermiFAST catalogue is
essentially complete for all 3PSC sources above 15-sigma, so FermiFAST
can quickly (in 90 seconds) generate a sample from the Fermi data
stream of the top third of sources that would appear a Fermi catalogue
using the full likelihood technique to construct the catalogue.

\begin{figure*}
\includegraphics[width=\columnwidth]{sign-comp}
\includegraphics[width=\columnwidth]{sign-dist}
\caption{Left: 3PSC Significances vs Fermi FAST.
  Right: Cumulative distribution of 3PSC significances for the entire 3PSC (green)
  and Fermi FAST (red).}
